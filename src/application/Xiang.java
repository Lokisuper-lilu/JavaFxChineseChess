...
class Xiang extends Qizi {
	...
	protected void loadimage() {
		...
		if (isRed == 1) {
			super.name = 11;
			if (isSceleted) {
				image = new Image(new File("Æå×ÓÍ¼Æ¬\\RBS.GIF").toURI().toString());
			} else {
				image = new Image(new File("Æå×ÓÍ¼Æ¬\\RB.GIF").toURI().toString());
			}
		} else {
			super.name = 12;
			if (isSceleted) {
				image = new Image(new File("Æå×ÓÍ¼Æ¬\\BBS.GIF").toURI().toString());
			} else {
				image = new Image(new File("Æå×ÓÍ¼Æ¬\\BB.GIF").toURI().toString());
			}
		}
