...
class Pao extends Qizi {
	...
	protected void loadimage() {
		...
		if (isRed == 1) {
			super.name = 9;
			if (isSceleted) {
				image = new Image(new File("Æå×ÓÍ¼Æ¬\\RCS.GIF").toURI().toString());
			} else {
				image = new Image(new File("Æå×ÓÍ¼Æ¬\\RC.GIF").toURI().toString());
			}
		} else {
			super.name = 10;
			if (isSceleted) {
				image = new Image(new File("Æå×ÓÍ¼Æ¬\\BCS.GIF").toURI().toString());
			} else {
				image = new Image(new File("Æå×ÓÍ¼Æ¬\\BC.GIF").toURI().toString());
			}
		}
